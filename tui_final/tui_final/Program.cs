...
namespace ParqueaderoApp
{
	...
	static class ValidadorDatos
	{
        ...
        public static string ValidarMarca(string prompt)
        {
            while (true)
            {
                Console.Write(prompt);
                ...
                if (partes.Length >= 2 && partes.Length <= 4)
                {
                    ...
                    foreach (string parte in partes)
                    {
                        if (parte.Length > 20 || !Regex.IsMatch(parte, @"^[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ ]+$"))
                            valido = false;
                    }
                    if (valido) return nombre;
                }

                MostrarError("Nombre inválido. Debe contener 2-4 partes");
            }
        }


        public static string ValidarColorVehiculo(string prompt)
        {
            while (true)
            {
                Console.Write(prompt);
                string color = Console.ReadLine().Trim();
                string[] partes = Regex.Replace(color, " +", " ").Split(' ');
		...
		public static string ValidarEmail(string prompt)
		{
			while (true)
			{
				...
				string patron = @"^([a-zA-Z][a-zA-Z0-9._]{1,19})@(gmail|yahoo|hotmail|pascualbravo|email)\.(com|co|es|edu\.co|com\.co)$";
				bool esValido = Regex.IsMatch(email, patron, RegexOptions.IgnoreCase);
				if (esValido)
				{
					return email;
				}
        ...
        public static int ValidarTipoVehiculo(string prompt)
        {
            while (true)
            {
                Console.Write(prompt);
                ...
                {
                    MostrarError("Tipo inválido. Debe ser '1' o '2'.");
                    continue;
                }
				return tipo;
            }
		...
		public static string ValidarHora(string prompt)
		{
			while (true)
			{
				...
				try
				{
					DateTime.ParseExact(hora, "HH:mm", null);
					return hora;
				}
				catch
				{
					MostrarError("Hora inválida. Formato: HH:MM");
				}
			}
		}
	...
	class ProgramaParqueadero
	{
		...
		static string SeleccionarSucursal()
		{
			...
			int tipoVehiculo = ValidadorDatos.ValidarTipoVehiculo("Tipo de vehículo siendo '1' Carro o '2' Moto(Carro/Moto): ");
            ...
            Cliente nuevoCliente = new Cliente
				{
                    ...
                    Marca = ValidadorDatos.ValidarMarca("Marca del vehículo: "),
					Modelo = ValidadorDatos.ValidarModelo(string.Format("Modelo (2000-{0}): ", DateTime.Now.Year + 1)),
					Color = ValidadorDatos.ValidarColorVehiculo("Color del vehículo: "),
                    Placa = ValidarPlacaUnica(tipoVehiculo),
                    TipoTarifa = ValidarTipoTarifa(),
					HoraEntrada = ValidadorDatos.ValidarHora("Hora de entrada (HH:MM): ")
				};
			...
			static string ValidarPlacaUnica(int tipoVehiculo)
			{
			while (true)
			{
				string placa;
				...
				if (!clientes.Any(c => c.Placa != null && c.Placa.ToUpper() == placa.ToUpper()))
				{
					return placa;
				}
            ...
            string placa;
            string mensaje = ("Ingrese el tipo de vehiculo siendo '1' Carro y '2' Moto: ");
			Console.Write(mensaje);
			int tipoveh = int.Parse(Console.ReadLine());
			if(tipoveh !=1 && tipoveh != 2)
			{
				...
				Menu();
					...
					if(tipoveh == 1)
			{
                placa = ValidadorDatos.ValidarPlacaCarro("Ingrese placa a buscar: ");

			}
			else
			{

			placa = ValidadorDatos.ValidarPlacaMoto("Ingrese placa a buscar: ");
			}
				...
				do
				{
					...
					switch (opcion)
					{
						...
						case '5':
							...
							break;
						case '6':
							cliente.Marca = ValidadorDatos.ValidarMarca("Nueva marca: ");
							break;
						case '7':
							cliente.Modelo = ValidadorDatos.ValidarModelo("Nuevo modelo: ");
							break;
						case '8':
							cliente.Color = ValidadorDatos.ValidarColorVehiculo("Nuevo color: ");
							break;
						case '9':
						cliente.Placa = ValidarPlacaUnica(cliente.TipoVehiculo);
							break;
						case 'T':
